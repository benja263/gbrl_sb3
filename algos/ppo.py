...
class PPO_GBRL(OnPolicyAlgorithm):
    ...
    def __init__(self, env: Union[GymEnv, str],
                 ...
                 policy_kwargs: Dict = {
                    ...
                    'gbrl_params': {
                        ...
                        'pg_optimizer': {
                            'pg_algo': 'SGD',
                            'pg_lr': 5.209483743331993e-03,
                            'pg_shrinkage': 0, # CPU only
                        }, 
                        'value_optimizer': {
                            'vf_algo': 'SGD',
                            'vf_lr': 0.009038987598789402,
                            'vf_shrinkage': 0, # CPU only
                        },
                    }
                 },
    ...
    def train(self) -> None:
        ...
        pg_lr, vf_lr = self.policy.get_schedule_learning_rates()
        self.logger.record("train/pg_learning_rate", pg_lr)
        self.logger.record("train/vf_learning_rate", vf_lr)
        ...
        # log_std_grad_maxs, log_std_grad_mins = [], []
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                theta, values, log_prob, entropy = self.policy.evaluate_actions_train(rollout_data.observations, actions)
                # Normalize advantage
                ...
                if self.policy_bound_loss_weight is not None and self.policy_bound_loss_weight > 0.0 and isinstance(self.action_space, spaces.Box):
                    vio_min = th.clamp(theta - self.bound_min, max=0)
                    vio_max = th.clamp(theta - self.bound_max, min=0)
                    violation = vio_min.pow_(2).sum(axis=-1) + vio_max.pow_(2).sum(axis=-1)
                    violation = 0.5 * self.policy_bound_loss_weight * th.mean(violation)
                ...
                theta_grad = theta.grad.detach().cpu().numpy() * num_samples # sample wise gradients in GBTs
                # GRAD CLIP - DO NOT USE PYTORCH CLIP AS IT IS NOT PER SAMPLE !!!!
                if self.normalize_policy_grads:
                    theta_grad = (theta_grad - np.mean(theta_grad, axis=0)) / (np.std(theta_grad, axis=0) + 1e-8)

                theta_grad_norms = np.linalg.norm(theta_grad, axis=1)
                if self.max_policy_grad_norm is not None and self.max_policy_grad_norm > 0.0:
                    theta_grad[theta_grad_norms > self.max_policy_grad_norm] = self.max_policy_grad_norm*theta_grad[theta_grad_norms > self.max_policy_grad_norm] / theta_grad_norms[theta_grad_norms > self.max_policy_grad_norm, np.newaxis]
                ...
                values_grad = values_pred.grad.detach().cpu().numpy() * num_samples # sample wise gradients i n GBTs
                if self.max_value_grad_norm is not None and self.max_value_grad_norm > 0.0:
                    values_grad = np.clip(values_grad, a_min=-self.max_value_grad_norm, a_max=self.max_value_grad_norm)
                ...
                self.policy.fit(rollout_data.observations, theta_grad, values_grad)
                ...
                theta_means.append(theta.mean(axis=0).mean().item())
                ...
                if not self.is_categorical:
                    observation_means.append(rollout_data.observations.mean(axis=0).mean().item())
        ...
        if len(theta_maxs) > 0:
            ...
            self.rollout_cntr += 1
            # if self.config.env.log_stats > 0 and self.rollout_cntr % self.config.env.log_stats == 0:
            #     self.generate_exp_report()
            ...
            # if log_std_grad_maxs:
            #     self.logger.record("param/log_std_grad_max", np.mean(log_std_grad_maxs))
            #     self.logger.record("param/log_std_grad_min", np.mean(log_std_grad_mins))
            if violations:
                self.logger.record("train/violations", np.mean(violations))
            ...
            self.logger.record("train/pg_boosting_iterations", iteration)
            self.logger.record("train/vf_boosting_iteration", value_iteration)
            self.logger.record("train/pg_num_trees", num_trees)
            self.logger.record("time/total_timesteps", self.num_timesteps)
            self.logger.record("train/vf_num_trees", value_num_trees)
            if not self.is_categorical:
                self.logger.record("param_input_output/observation_mean", np.mean(observation_means))
            self.logger.record("param_input_output/theta_mean", np.mean(theta_means))
            if log_std_s:
