...
class AWR_GBRL(OffPolicyAlgorithm):
    ...
    def shared_models_train(self, gradient_steps: int, batch_size: int) -> None:
        ...
        value_losses, pg_losses, entropy_losses = [], [], []
        ...
        for gradient_step in range(gradient_steps):
            ...
            theta, values, log_prob, entropy = self.policy.evaluate_actions_train(replay_data.observations, actions)
            # Normalize advantage
            ...
            loss.backward()
            theta_grad = theta.grad.detach().cpu().numpy() * n_samples
            ...
            values_grad = values.grad.detach().cpu().numpy() * n_samples
            ...
            self.policy.fit(replay_data.observations, theta_grad, values_grad)
    ...
    def separate_models_train(self, batch_size: int) -> None:
        ...
        for gradient_step in range(self.value_gradient_steps):
            ...
            pred_values = th.tensor(pred_values, requires_grad=True)

            value_loss = 0.5*F.mse_loss(pred_values, replay_data.returns)
            value_loss.backward()

            values_grad = pred_values.grad.detach().cpu().numpy() * n_samples
            if self.max_value_grad_norm is not None and self.max_value_grad_norm > 0.0:
                values_grad = np.clip(values_grad, a_min=-self.max_value_grad_norm, a_max=self.max_value_grad_norm)

            value_losses.append(value_loss.item())
            self.policy.fit_critic(replay_data.observations, values_grad)
        ...
        for gradient_step in range(self.policy_gradient_steps):
            ...
            theta, _, log_prob, entropy = self.policy.evaluate_actions_train(replay_data.observations, actions)
            ...
            self.policy.fit_actor(replay_data.observations, theta_grad)    
