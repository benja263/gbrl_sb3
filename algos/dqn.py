...
from gbrl.gbrl import DiscreteCriticGBRL
...
class DQN_GBRL(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            current_q_values = self.q_model(replay_data.observations)

            # Retrieve the q-values for the actions from the replay buffer
            current_max_q_values = th.gather(current_q_values, dim=1, index=replay_data.actions.long())

            # Compute Huber loss (less sensitive to outliers)
            # loss = F.smooth_l1_loss(current_max_q_values, target_q_values, reduction='none')
            loss = F.mse_loss(current_max_q_values, target_q_values, reduction='none')
            losses.append(loss.mean().item())

            # Optimize the policy
            loss.backward(th.ones((len(current_q_values), 1)))
            q_grad = current_q_values.grad.detach().cpu().numpy()
            # # Calculate mean of non-zero values per column
            # column_sums = q_grad.sum(axis=0)
            # non_zero_counts = np.count_nonzero(q_grad, axis=0)
            # means = np.true_divide(column_sums, non_zero_counts, where=(non_zero_counts!=0))

            # # Replace zeros with the calculated means, except for columns with only zeros
            # mask = (q_grad == 0) & (non_zero_counts != 0)
            # q_grad[mask] = np.where(mask, means, q_grad)[mask]

            # GRAD CLIP - DO NOT USE PYTORCH CLIP AS IT IS NOT PER SAMPLE !!!!
            if self.normalize_q_grads:
                q_grad = (q_grad - np.mean(q_grad, axis=0)) / (np.std(q_grad, axis=0) + 1e-8)

            q_grad_norms = np.linalg.norm(q_grad, axis=1)
            if self.max_q_grad_norm is not None and self.max_q_grad_norm > 0.0:
                q_grad[q_grad_norms > self.max_q_grad_norm] = self.max_q_grad_norm*q_grad[q_grad_norms > self.max_q_grad_norm] / q_grad_norms[q_grad_norms > self.max_q_grad_norm, np.newaxis]

            # Fit GBRL model on gradients - Optimization step
            self.q_model.fit(replay_data.observations, q_grad)
