...
from gbrl import ActorGBRL, ContinuousCriticGBRL
...
class ContinuousCritic(BaseModel):
    ...
    def __init__(
        ...
        for _ in range(n_critics):
            ...
            q_model = ContinuousCriticGBRL(tree_struct=tree_struct, 
                                            output_dim=action_dim + self.theta_dim, 
                                            critic_optimizer=copy.deepcopy(critic_optimizer),
    ...
    def forward(self, obs: th.Tensor, actions: th.Tensor, train_mode: bool) -> Tuple[th.Tensor, ...]:
        q_s = []
        for q_net in self.q_models:
            theta = q_net(obs)
            weights, bias = theta[:, :-self.theta_dim], theta[:, -self.theta_dim:] 
            if train_mode:
                q_s.append((weights, bias))
            else:
                weights = th.tensor(weights).float()
                bias = th.tensor(bias).float()
    ...
    def fit_target(self, obs: th.Tensor, tau: float):
        obs = obs.detach().cpu().numpy().astype(np.single)
        for i in range(self.n_critics):
            theta = self.q_models[i](obs)
            if self.q_models[i].target_model is not None:
                theta_target = self.q_models[i].predict_target(obs)
                poly_theta = (1 - tau)*theta_target + tau*(theta)
                self.q_models[i].fit_target(obs, poly_theta)
            else:
                self.q_models[i].fit_target(obs, theta)

    def predict_target(self, obs: th.Tensor, actions: th.Tensor):
        q_s = []
        obs = obs.detach().cpu().numpy().astype(np.single)
        for q_net in self.q_models:
            if q_net.target_model is not None:
                theta = q_net.predict_target(obs)
            else:
                theta = q_net(obs)
            weights, bias = theta[:, :-self.theta_dim], theta[:, -self.theta_dim:] 
            weights = th.tensor(weights).float()
    ...
    def fit(self, obs: th.Tensor, critic_grads: List[np.array]):
        for idx in range(self.n_critics):
            self.q_models[idx].fit(obs, critic_grads[idx])
...
class Actor(BasePolicy):
    ...
    def __init__(
        ...
        self.model = ActorGBRL(tree_struct=tree_struct, 
    ...
    def get_action_dist_params(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Dict[str, th.Tensor]]:
        ...
        mean_actions, log_std = theta[:, :self.action_dim], theta[:, self.action_dim:]
        # Unstructured exploration (Original implementation)
        # Original Implementation to cap the standard deviation
        log_std = th.clamp(log_std, LOG_STD_MIN, LOG_STD_MAX)
        mean_actions, log_std = th.tensor(mean_actions, requires_grad=True), th.tensor(log_std, requires_grad=True)
